 ...
 * Devuelve un motivo claro si no puede, para diagnosticar sin adivinar.
 *
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!uid || !email) {
      res.status(400).json({ active: false, reason: 'Faltan uid o email' });
      return;
    }
      ...
      res.status(200).json({
        ...
        reason:
          customers.data.length === 0
            ? `No hay ningún cliente en Stripe con el email ${email}`
            : 'El cliente existe pero no tiene ninguna suscripción activa',
      });
      return;
    }
    ...
    // Devuelve el error real (p. ej. clave de Stripe inválida) para diagnosticar.
    res.status(500).json({ active: false, reason: e.message || String(e) });
  }
}
